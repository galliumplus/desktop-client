...
namespace Couche_Métier.Log
{
    ...
    /// </summary>
    public class LogCompteToTxt : ILog
    {
        ...
        public void registerLog(CategorieLog categorieLog, object itemModif, User author)
        {
            ...
            switch (categorieLog)
            {
                case CategorieLog.UPDATE:
                    message = $"{u.NomComplet.ToUpper()} a été modifié";
                    break;
                case CategorieLog.CREATE:
                    message = $"{u.NomComplet.ToUpper()} a été créé";
                    break;
                case CategorieLog.DELETE:
                    message = $"{u.NomComplet.ToUpper()} a été supprimé";
                    break;

            }      
            string log = $"{DateTime.Now}|COMPTE|{message}|{author.NomComplet}|{categorieLog}";
